...
func TestExamplesComplete(t *testing.T) {
	...
	assert.Contains(t, kmsKeyArn, "arn:aws:kms:us-east-2")

	// Run `terraform output` to get the value of an output variable
	kmsKeyAliasName := terraform.Output(t, terraformOptions, "kms_key_alias_name")
	kmsKeyExpectedAliasName := "alias/eg-test-chamber-s3-role-" + randId
	// Verify we're getting back the outputs we expect
	assert.Equal(t, kmsKeyExpectedAliasName, kmsKeyAliasName)

	// Run `terraform output` to get the value of an output variable
	s3BucketId := terraform.Output(t, terraformOptions, "bucket_id")
	expectedS3BucketId := "eg-test-chamber-s3-role-" + randId
	// Verify we're getting back the outputs we expect
	assert.Equal(t, expectedS3BucketId, s3BucketId)

	// Run `terraform output` to get the value of an output variable
	roleName := terraform.Output(t, terraformOptions, "role_name")
	expectedRoleName := "eg-test-chamber-s3-role-" + randId
	// Verify we're getting back the outputs we expect
